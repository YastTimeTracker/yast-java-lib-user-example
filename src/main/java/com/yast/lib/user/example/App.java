...
public class App 
{
    public static void main( String[] args ) throws YastLibBadResponseException, YastLibApiException
    {
    	Yast yast;
		...
		App.log("Fetched " + projects.size() + " projects and " + folders.size() + " folders");
		
		
		// Add a project
		YastProject project = new YastProject("Test project");
		project.setInternalId(555); // local id, can be used to keep track of objects not already synced with Yast.com
		App.log("Project local id: " + project.getInternalId() + " remote id: " + project.getId());
		yast.add(project);
		App.log("Project local id: " + project.getInternalId() + " remote id: " + project.getId()); // The remote id will be updated
		...
		yast.add(record);
		App.log("Record local id: " + record.getInternalId() + " remote id: " + record.getId()); // The remote id will be updated
		App.log("Record duration is: " + (record.getEndTime() - record.getStartTime()));
		...
		App.log("Fetched " + recordsLastWeek.size() + " records for the last week");
		...
		ArrayList<YastRecord> recordsProject = yast.getRecords(null, null, projectIds, from, to);
		App.log("Fetched " + recordsProject.size() + " records for the last week for project id " + project.getId());
		...
		ArrayList<YastRecord> recordsSingle = yast.getRecords(null, ids, null, from, to);
		App.log("Fetched record " + recordsSingle.get(0).getId() + " from server");
